...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	const int delta_change_weight = 1000000;
    ...
    {
       fg[0] += cte_weight*CppAD::pow(vars[cte_start + t], 2); // cte
       fg[0] += epsi_weight*CppAD::pow(vars[epsi_start + t], 2); // epsi
       fg[0] += v_weight*CppAD::pow(vars[v_start + t] - v_ref, 2); // This is to avoid
     }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = (states_no*N) + (actuators_no*(N-1));
  // TODO: Set the number of constraints
  size_t n_constraints = (states_no*N);
